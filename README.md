...
**Creating SQL database** - Initially we had a csv file that required cleaning. After it was cleaned, we used PG Admin4 to create a database with the goal of uploading the database to Jupyter notebook to run our supervised machine learning algorithm. We built a schema to account for the gender data type and smoker or non-smoker classification but after careful consideration with our group we changed it to binary. Finally, once the csv was uploaded and connection made on jupyter notebook we were able to generate a random forest supervised machine learing to determine if we had efficient or inefficient sleep. Our model yielded an 83%, however, that took some modifications to the ensemble of decision trees.
